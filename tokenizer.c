 ...
 */
char *next_token(char **str_ptr, const char *delim, char * quote)
{
    if (*str_ptr == NULL) {
        return NULL;
    }
    ...
    if (quote_start < tok_end)
    {
        ...
        *quote = quotes[0];
        quotes[1] = '\0';
        size_t offset1 = quote_start + 1;
        size_t quote_end1 = strcspn(*str_ptr + offset1, quotes) + offset1;
        tok_end = strcspn(*str_ptr + quote_end1, delim) + quote_end1 - tok_start;
